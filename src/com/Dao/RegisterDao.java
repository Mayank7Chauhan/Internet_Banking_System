...
public class RegisterDao {
	...
	public boolean validateUser(Login lobj)
	{
		...
		try
		{
			PreparedStatement ps=con.prepareStatement("select * from Banks.Account where username=? and password=?");
	...
	public int createAccount(Register r)
	{
		...
		try
		{
			pstate=con.prepareStatement("insert into Banks.Account values(?,?,?,?,?)");
	...
	public List<Register> DisplayAccount()
	{
		...
		try
		{
			...
			rs=state.executeQuery("select * from Banks.Account");
	...
	public List<Register> SearchAccount(int accno)
	{
		...
		try
		{
			PreparedStatement ps=con.prepareStatement("select * from Banks.Account where Account_Number=?");
			ps.setInt(1,accno);
			ResultSet rs=ps.executeQuery();
			if(rs.next())
			{
	...
	public int UpdateAccount(int accno,double accbal)
	{
		...
		con=m.getConnection();
		try {
			pstate=con.prepareStatement("update Banks.Account set Account_Balance=? where Account_Number=?");
	...
	public int DeleteAccount(int accno)
	{
		...
		con=m.getConnection();
		try {
			pstate=con.prepareStatement("delete from Banks.Account where Account_Number=?");
